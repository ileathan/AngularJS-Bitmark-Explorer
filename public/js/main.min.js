...
var defaultLanguage = localStorage.getItem("insight-language") || "en", defaultCurrency = localStorage.getItem("insight-currency") || "BTM";
angular.module("insight", ["ngAnimate", "ngResource", "ngRoute", "ngProgress", "ui.bootstrap", "ui.route", "monospaced.qrcode", "gettext", "angularMoment", "insight.system", "insight.socket", "insight.blocks", "insight.transactions", "insight.address", "insight.search", "insight.status", "insight.connection", "insight.currency", "insight.messages", "insight.syncer"]), angular.module("insight.system", []), angular.module("insight.socket", []), angular.module("insight.syncer", []), angular.module("insight.blocks", []), angular.module("insight.transactions", []), angular.module("insight.address", []), angular.module("insight.search", []), angular.module("insight.status", []), angular.module("insight.connection", []), angular.module("insight.currency", []), angular.module("insight.messages", []), angular.module("insight.address").controller("AddressController", function ($scope, $rootScope, $routeParams, $location, Global, Address, getSocket) {
    $scope.global = Global;
...
}), angular.module("insight.blocks").controller("BlocksController", function ($scope, $window, $rootScope, $routeParams, $location, Global, Block, Blocks, BlockByHeight) {
    ...
    // ~AxE toggle all the transactions. (send a broadcast out).
    $scope.emitMe = function () {
        $rootScope.$broadcast('toggle_all')
        $scope.transOpened = !$scope.transOpened
    };



    var _formatTimestamp = function (date) {
        var yyyy = date.getUTCFullYear().toString(), mm = (date.getUTCMonth() + 1).toString(), dd = date.getUTCDate().toString();
...
}), angular.module('insight.syncer', [], function($provide) {
    ...
    $rootScope.currency.getConvertion = function (value) {
        if (value = 1 * value, !isNaN(value) && "undefined" != typeof value && null !== value) {
            ...
            var response;
            return "USD" === this.symbol ? response = _roundFloat(value * this.factor, 2) : "Marks" === this.symbol ? (this.factor = 1e3, response = _roundFloat(value * this.factor, 5)) : "bits" === this.symbol ? (this.factor = 1e6, response = _roundFloat(value * this.factor, 2)) : (this.factor = 1, response = value), 1e-7 > response && (response = response.toFixed(8)), response + " " + this.symbol
        }
        return "value error"
        ...
        }) : $rootScope.currency.factor = "Marks" === currency ? 1e3 : "bits" === currency ? 1e6 : 1
    ...
    $scope.version = _getVersion(), $scope.availableLanguages = [{name: "English", isoCode: "en"}, {
        name: "Spanish",
        isoCode: "es"
    }, {name: "Japanese", isoCode: "ja"}], $scope.setLanguage = function (isoCode) {
...
angular.module("insight.system").controller("IndexController", function ($scope, Global, getSocket, Blocks) {
    ...
    var cameraInput, video, canvas, $video, context, localMediaStream, $searchInput = angular.element(document.getElementById("search")), _scan = function (evt) {
        if ($scope.isMobile) {
            ...
            if (1 === files.length && 0 === files[0].type.indexOf("image/")) {
                ...
                reader.onload = function () {
                    return function () {
                        ...
                        mpImg.render(canvas, {maxWidth: 200, maxHeight: 200, orientation: 6}), setTimeout(function () {
    ...
    qrcode.callback = function (data) {
        _scanStop();
        var str = 0 === data.indexOf("bitmark:") ? data.substring(8) : data;
    ...
    $scope.search = function () {
        var q = $scope.q;
        $scope.badQuery = !1, $scope.loading = !0, Block.get({blockHash: q}, function () {
            _resetSearch(), $location.path("block/" + q)
        }, function () {
            Transaction.get({txId: q}, function () {
                _resetSearch(), $location.path("tx/" + q)
            }, function () {
                Address.get({addrStr: q}, function () {
...
}), angular.module("insight.transactions").controller("transactionsController", function ($scope, $window, $rootScope, $routeParams, $location, Global, Transaction, TransactionsByBlock, TransactionsByAddress) {
    $scope.global = Global, $scope.loading = !1, $scope.loadedBy = null;
    if (document.getElementById("linker").value == "1") {
        document.getElementById("linker").value = "0";
        document.getElementById("back_one_diff").href = "#";
        document.getElementById("back_one_block").href = "#";
        document.getElementById("forward_one_block").href = "/block-index/1";
        document.getElementById("forward_one_diff").href = "/block-index/720";
    }

    var pageNum = 0, pagesTotal = 1, COIN = 1e8, _aggregateItems = function (items) {
        if (!items)return [];
        for (var l = items.length, ret = [], tmp = {}, u = 0, i = 0; l > i; i++) {
            var notAddr = !1;
            if (items[i].scriptSig && !items[i].addr && (items[i].addr = "Unparsed address [" + u++ + "]", items[i].notAddr = !0, notAddr = !0), items[i].scriptPubKey && !items[i].scriptPubKey.addresses && (items[i].scriptPubKey.addresses = ["Unparsed address [" + u++ + "]"], items[i].notAddr = !0, notAddr = !0), items[i].scriptPubKey && items[i].scriptPubKey.addresses.length > 1)items[i].addr = items[i].scriptPubKey.addresses.join(","), ret.push(items[i]); else {
        ...
        blockHeight = parseInt(document.getElementById("blocklink").innerHTML),
        blockTotal = parseInt(document.getElementById("blockTotal").innerHTML);

        document.getElementById("back_one_diff").href = "/block-index/" + ( blockHeight-(blockHeight%720)-1 ),
        document.getElementById("back_one_block").href = "/block-index/" + (blockHeight - 1),
        document.getElementById("forward_one_block").href = "/block-index/" + (blockHeight + 1),
        document.getElementById("forward_one_diff").href = "/block-index/" + ( blockHeight-(blockHeight%720)+720 ),
        document.getElementById("linker").value = blockHeight; // Doesn't work if the daemon cant read every tx ~AxE~

        if ( (blockHeight+720) > blockTotal ) {
            document.getElementById("forward_one_diff").href = '#';
        } if ( (blockHeight-720) < 0 ) {
            document.getElementById("back_one_diff").href = '#';
        } if (blockTotal-1 < 0 ) {
            document.getElementById("back_one_block").href = '#';
        } if ( (blockHeight+1 > blockTotal) ) {
            document.getElementById("forward_one_block").href = '#';
        } if (!blockHeight) {
            document.getElementById("linker").value = '0';
        };
    }, _byBlock = function () {
    ...
    $scope.$on('toggle_all', function () {
        $scope.itemsExpanded = !$scope.itemsExpanded;
    });
    ...
    return $resource("/api/addr/:addrStr/?noTxList=1", {addrStr: "@addStr"}, {
        get: {
            method: "GET",
            interceptor: {
    ...
    return $resource("/api/block/:blockHash", {blockHash: "@blockHash"}, {
        get: {
            method: "GET",
            interceptor: {
                response: function (res) {
...
ScopedSocket.prototype.removeAllListeners = function (opts) {
    ...
    var socket = this.socket, $rootScope = this.$rootScope;
    ...
    return $resource("/api/status", {q: "@q"})
    ...
    return $resource("/api/tx/:txId", {txId: "@txId"}, {
        get: {
    ...
    return $resource("/api/txs", {block: "@block"})
}).factory("TransactionsByAddress", function ($resource) {
    return $resource("/api/txs", {address: "@address"})
...
angular.module("insight").directive("scroll", function ($window) {
    ...
    gettextCatalog.setStrings("es", {
        ...
        "Bitmark node information": "Información del nodo Bitmark",
        Block: "Bloque",
        "Block Reward": "Bloque Recompensa",
        Blocks: "Bloques",
        "Bytes Serialized": "Bytes Serializados",
        "Can't connect to bitmarkd to get live updates from the p2p network. (Tried connecting to bitmarkd at {{host}}:{{port}} and failed.)": "No se pudo conectar a bitmarkd para obtener actualizaciones en vivo de la red p2p. (Se intentó conectar a bitmarkd de {{host}}:{{port}} y falló.)",
        ...
        "Last Block Hash (Bitmarkd)": "Último Bloque Hash (Bitmarkd)",
        ...
        "Best Block": "最良ブロック",
        "Bitmark comes with a way of signing arbitrary messages.": "Bitmarkには任意のメッセージを署名する昨日が備わっています。",
        "Bitmark node information": "Bitmarkノード情報",
        Block: "ブロック",
        "Block Reward": "ブロック報酬",
        Blocks: "ブロック",
        "Broadcast Raw Transaction": "生のトランザクションを配信",
        "Bytes Serialized": "シリアライズ後の容量 (バイト)",
        "Can't connect to bitmarkd to get live updates from the p2p network. (Tried connecting to bitmarkd at {{host}}:{{port}} and failed.)": "P2Pネットワークからライブ情報を取得するためにbitmarkdへ接続することができませんでした。({{host}}:{{port}} への接続を試みましたが、失敗しました。)",
        ...
        Input: "入力",
        "Last Block": "直前のブロック",
        "Last Block Hash (Bitmarkd)": "直前のブロックのハッシュ値 (Bitmarkd)",
        ...
        "This form can be used to broadcast a raw transaction in hex format over\n        the Bitmark network.": "このフォームでは、16進数フォーマットの生のトランザクションをBitmarkネットワーク上に配信することができます。",
        "This form can be used to verify that a message comes from\n        a specific Bitmark address.": "このフォームでは、メッセージが特定のBitmarkアドレスから来たかどうかを検証することができます。",
        "Time Offset": "時間オフセット",
        Timestamp: "タイムスタンプ",
        Today: "今日",
        "Total Amount": "Bitmark総量",
        "Total Received": "総入金額",
        "Total Sent": "総送金額",
